...
class Bible:
    ...
    def book(self, name):
        return "\n".join(
            line for line, normalized in self.content if re.search(rf"^{name}", normalized, re.IGNORECASE)
        ).rstrip()
    ...
    def verse(self, book, chapter, verse):
        return "\n".join(
            line
            for line, normalized in self.content
            if re.search(rf"^{book}.* {chapter}:{verse} (.*)", normalized, re.IGNORECASE)
        ).rstrip()
    ...
    def verseSlice(self, book, chapter, start, end):
        ...
        return "\n".join(
            line
            for line, normalized in self.content
            if re.findall(rf"^{book}.* {chapter}:(?=({verses}))", normalized, re.IGNORECASE)
        ).rstrip()


    def _filter(self, value):
        return [(line, normalized) for line, normalized in self.content if re.search(rf"\b{value}\b", normalized, re.IGNORECASE)]
    ...
    def count(self, value):
        if target := value.lower():
            return sum(normalized.lower().count(target) for _, normalized in self._filter(value))
        return 0
                ...
                match value.split(":"):
                    ...
                    case [chapter, verse]:
                        match verse.split("-"):
                            ...
                            case [start, end]:
                                return self.verseSlice(book, int(chapter), int(start), int(end))
